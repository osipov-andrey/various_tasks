...
try:
    rndis = RNDIS(ip=settings.get('ip'), start=int(settings.get('ip_start')), end=int(settings.get('ip_end')))
except Exception:
    rndis = RNDIS()
net_config = rndis.ifconfig()

sse_host = net_config[0]
sse_port = 5000
picoweb_debug = True
...
class Publisher:
    ...
    async def subscribe(self, channel='default channel'):
        ...
        while len(subscribers_list) >= max_clients:
            subscriber_to_remove = subscribers_list[0]
            id_ = subscriber_to_remove.ID
            # Удаляем первого подписчика, если список полон
            await self.publish('DISCONNECT {}'.format(id_))
            await sleep(0.5)
    ...
    def clear_clients_list(self, channel='default channel'):
        subscribers_list = self.get_subscribers(channel)
        subscribers_list.clear()
        print('SUBSCRIBERS LIST IS CLEAR')
...
async def subscribe(req, resp):
    ...
    await start_response(resp, content_type='text/event-stream')

    try:
        async for data in subscriber_aiterator:
            message = message_to_client.format(data)

            if data.strip().startswith('DISCONNECT'):
                if data.split()[1] == str(subscriber_id):
                    # Сигнал о том, что сервер отключает клиента
                    await resp.awrite("event: disconnect\ndata: {}\n\n".format('too many connections'))
                    publisher.unsubscribe(subscriber_id)
                    print("Client {} was disconnected".format(subscriber_id))
                    return
                else:
                    # Сигнал о том, что сервер отключает какого-то другого клиента
                    message = "event: data\ndata: {}\n\n".format(data)

            print(message)

            try:
                await resp.awrite(message)
                GLED.on()
                await sleep(1)
                GLED.off()
...
async def send_pseudo_id(req, resp):
    print('Free mem at work:', mem_free())
...
if __name__ == '__main__':
    ...
    print(net_config)
    print('Free mem at start:', mem_free())
